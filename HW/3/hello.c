...
int main(int argc, char *argv[])
{
	...
	for(unsigned int t=0; t<timeSteps; t++)
	{
		...
		if(numNodes > 1)
		{
			...
			else if(nodeRank == (numNodes-1))
			{
				...
				for(unsigned int column =1; column < N-1; column++)
				{
					tempArray[rowPerSection-1][column] = (upperGhostRow[column-1]+upperGhostRow[column]+upperGhostRow[column]+
	...
	if(numNodes > 1)
	{
		unsigned double localSum = 0; 
		unsigned double globalSum = 0; 
