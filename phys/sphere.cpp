...
// Version of the function with xyz offset
Shape* randomShape(double radMin, double radMax, double massMin, double massMax, cart tMaxPos, cart tOffsetPos, cart tMaxVel)
{
	...
	cart tPos = {	(rand()*(2*tMaxPos.x)/RAND_MAX - tMaxPos.x - tOffsetPos.x), 
					(rand()*(2*tMaxPos.y)/RAND_MAX - tMaxPos.y - tOffsetPos.y), 
					(rand()*(2*tMaxPos.z)/RAND_MAX - tMaxPos.z - tOffsetPos.z)};
